...
```
dbx execute --cluster-name=<name of interactive cluster> --job=telco_churn_mlops-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=telco_churn_mlops-sample-integration-test --files-only
dbx launch --job=telco_churn_mlops-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `telco_churn_mlops/__init__.py` file and tag the current code version:
```
